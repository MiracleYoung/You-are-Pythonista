...
## Question
将输入的两个字符串合并，要求是下标为奇数的字符和下标为偶数的字符分别从小到大排序。
...
while True:
    try:
        ...
        for i in range(len(even)):
            ...
            if len(odd) != i:
                if len(odd) != i:
                    ss += dic[int(bin(dic.index(odd[i].____)).replace('0b', '').rjust(4, '0')[::-1], 2)]
                else:
                    ss += odd[i]
        print(ss)
    except:
...
%!A. enumerate, sort, sort, lower()!%

%!B. enumerate, sorted, sorted, upper()!%

%!C.iterms, sorted, sorted, upper()!%

%!D. iterms, sort, sort, lower()!%
