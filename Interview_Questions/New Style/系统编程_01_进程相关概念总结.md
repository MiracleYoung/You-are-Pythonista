...
%!A. 程序运行在操作系统上的一个实例，就称之为进程!%

%!B. 进程间不共享全局变量!%

%!C. 使用 Queue 来进行进程之间的通信!%

%!D. 使用 Pool 创建进程的时候，就需要用到 multiprocessing.Queue()!%
...
如果要使用 Pool 创建进程，就需要使用 multiprocessing.Manager() 中的 Queue(),而不是multiprocessing.Queue(),否则会得到如下的错误信息：
